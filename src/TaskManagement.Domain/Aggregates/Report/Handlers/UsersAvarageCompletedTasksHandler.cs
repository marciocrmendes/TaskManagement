...
namespace TaskManagement.Domain.Aggregates.Project.Handlers
{
    public class UsersAvarageCompletedTasksHandler(IProjectRepository projectRepository) :
        IRequestHandler<GetUsersAvarageCompletedTasksQuery, GenericResponseList<UsersAvarageCompletedTasksReportResponse>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var projects = await projectRepository.Table
                .AsNoTracking()
                .Include(p => p.Tasks)
            ...
            var result = userIdList.Select(userId => 
            {
                var tasks = projects
                ...
                var completedTasksCount = tasks
                    .Count(t => t.Status == TaskStatusEnum.Completed &&
